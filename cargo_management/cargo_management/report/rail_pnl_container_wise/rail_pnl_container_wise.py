...
def get_data(filters, expense_types):
    ...
    data_query = f"""
        ...
        WHERE
            e.container_number IN (SELECT DISTINCT c.container_number FROM `tabFPL Perform Middle Mile` pm JOIN `tabExpenses cdt` c ON pm.name = c.parent WHERE pm.departure_time BETWEEN %(from_date)s AND %(to_date)s)
    """
